...
const currentPostReducer = createReducer(
  ...
  on(CurrentPostActions.postSuccess, (state, { post }) => ({
    ...state,
    post: post,
    loading: false,
    loaded: true,
  })),
  on(CurrentPostActions.transferPost, (state, { post }) => ({
    ...state,
    post: post,
  })),
  on(CurrentPostActions.commentsSuccess, (state, { comments }) => ({
    ...state,
    comments: comments,
  })),
  on(CurrentPostActions.resetState, () => CurrentPostInitial),
  on(CurrentPostActions.addCommentSuccess, (state, { comment }) => ({
    ...state,
